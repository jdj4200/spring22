  ...
  *)
  ...
  Fixpoint run (fuel: nat) (v1: valuation) (c: cmd) (v2: valuation): Prop :=
    ...
    | S fuel' =>
      ...
      | While e c1 =>
        (exists r v', interp e v1 r /\ r <> 0 /\ run fuel' v1 c1 v' /\ run fuel' v' c v2) \/
        (interp e v1 0 /\ v1 = v2)
      end
    end.
  ...
  Proof.
    ...
    cases H.
    propositional.
    apply interp_to_values in H0.
    apply IHfuel in H2.
    apply EvalIfTrue with x.
    linear_arithmetic.
    assumption.
    assumption.
    ...
    apply IHfuel in H1.
    apply IHfuel in H3.
    apply EvalWhileTrue with (v' := x0) (ret := x).
    linear_arithmetic.
    assumption.
    assumption.
    assumption.
  ...
  Definition WRunWhileTrue_statement : Prop :=
    forall v v1 v2 c1 e a,
      a <> 0 ->
      interp e v a ->
      wrun v c1 v1 ->
      wrun v1 (While e c1) v2 ->
      wrun v (While e c1) v2.
  ...
  Proof.
    ...
    apply WRunSkip.
    apply WRunAssign.
    apply values_to_interp.
    assumption.
    eapply WRunSeq; eauto.
    eapply WRunIfTrue.
    assert (ret <> 0) by linear_arithmetic.
    apply H2.
    apply values_to_interp.
    assumption.
    assumption.
    eapply WRunIfFalse; eauto.
    apply values_to_interp.
    assumption.
